...
async function cargarModelosFaceAPI() {
    // Usar CDN para los modelos (más confiable)
    ...
    try {
        ...
        updateStatus("Error cargando modelos. Recargando...", "fa-exclamation-triangle");
...
async function iniciarSistema() {
    try {
        ...
        updateStatus("Solicitando cámara...", "fa-spinner fa-pulse");
...
async function cargarUsuariosDesdeExcel() {
    try {
        console.log("📥 Cargando usuarios desde:", SCRIPT_URL_USUARIOS);
        
        // Intentar con timeout
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 10000);
        
        const response = await fetch(SCRIPT_URL_USUARIOS, { signal: controller.signal });
        ...
        if (Array.isArray(data)) {
            usuariosRegistrados = data.filter(user => user.name && user.name !== "").map(user => ({
                ...
                descriptor: user.faceDescriptor ? new Float32Array(JSON.parse(user.faceDescriptor)) : null
            }));
        } else if (data.error) {
        ...
        usuariosRegistrados = [];
        usuariosFiltrados = [];
        document.getElementById('userList').innerHTML = '<div class="empty-message"><i class="fas fa-exclamation-triangle"></i> Error cargando usuarios<br><small>Verifica la URL del script</small></div>';
    }
}
...
function buscarCoincidencia(descriptorActual) {
    ...
    // Filtrar usuarios que tienen descriptor válido
    const usuariosConDescriptor = usuariosRegistrados.filter(u => u.descriptor);
    if (usuariosConDescriptor.length === 0) return { label: "Desconocido", datos: null };
    
    const labeledDescriptors = usuariosConDescriptor.map(u => new faceapi.LabeledFaceDescriptors(u.name, [u.descriptor]));
...
function filtrarUsuarios() {
    ...
    if (searchTerm === "") {
        ...
        usuariosFiltrados = usuariosRegistrados.filter(user => 
            ...
            (user.empresa && user.empresa.toLowerCase().includes(searchTerm))
        );
    }
        ...
        html += `
            <div class="user-item ${isSelected ? 'selected' : ''}" onclick="seleccionarUsuario('${user.id}')">
                <div class="user-item-info">
                    <div class="user-item-name"><i class="fas fa-user"></i> ${user.name || 'Sin nombre'}</div>
                    <div class="user-item-rut"><i class="fas fa-id-card"></i> ${user.rut || 'Sin RUT'}</div>
                    <div class="user-item-empresa"><i class="fas fa-building"></i> ${user.empresa || 'Sin empresa'}</div>
                </div>
...
// Iniciar el sistema cuando la página cargue
